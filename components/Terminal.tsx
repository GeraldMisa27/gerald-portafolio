...
function buildCommands(projects: Project[]) {
    return {
        help: (): Line[] => [
            { type: "info" as const, text: "Comandos disponibles:" },
            { type: "output" as const, text: "  projects    → ver proyectos" },
            { type: "output" as const, text: "  skills      → stack técnico" },
            { type: "output" as const, text: "  experience  → experiencia laboral" },
            { type: "output" as const, text: "  contact     → información de contacto" },
            { type: "output" as const, text: "  about       → sobre mí" },
            { type: "output" as const, text: "  clear       → limpiar terminal" },
            { type: "output" as const, text: "  exit        → cerrar" },
        ],
        projects: (): Line[] =>
        ...
        skills: (): Line[] => [
            { type: "info" as const, text: "Stack técnico:" },
            ...STACK.map((s) => ({
        ...
        contact: (): Line[] => [
            { type: "output" as const, text: `Email:    ${SITE.email}` },
            { type: "success" as const, text: `GitHub:   ${SITE.github}` },
        ],
        about: (): Line[] => [
            { type: "output" as const, text: `${SITE.fullName} — ${SITE.role}` },
            { type: "output" as const, text: "Ing. Automático (CUJAE) → Frontend por pasión." },
            { type: "output" as const, text: "2.5 años en producción · Aprende desde docs oficiales." },
        ],
    };
}
export default function Terminal({
    ...
    const COMMANDS = buildCommands(projectList);
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (COMMANDS[cmd as keyof typeof COMMANDS]) {
            newLines.push(...COMMANDS[cmd as keyof typeof COMMANDS]());
        } else {
            newLines.push({
                type: "error",
